...
namespace Application.Features.Order.Queries.GetAllOrders
{
    ...
    public class GetAllOrdersHandler : IRequestHandler<GetAllOrdersQuery, Result<List<OrderDto>>>
    {
        ...
        public GetAllOrdersHandler(IUnitOfWork uow)
        {
            _uow = uow;
        }
        ...
        public async Task<Result<List<OrderDto>>> Handle(GetAllOrdersQuery request, CancellationToken cancellationToken)
        {
            var order = await _uow.OrderRepository
                .GetAllWithItemsAsync(cancellationToken);

            if (!order.Any())
                return Result<List<OrderDto>>.Success(new List<OrderDto>(), "Order not found");
            ...
            return Result<List<OrderDto>>.Success(dto, "Orders fetched successfully!");
        }
    }
}
