...
namespace Application.Features.Authentication.Command.Login
{
    public class LoginHandler : IRequestHandler<LoginCommand, Result<LoginResponse>>
    {
        ...
        {
            logger.LogInformation("LoginHandler called with Identifier: {Identifier}", request.Identifier);
            ...
            logger.LogInformation($"User {loginUser.User!.UserName} UserId : {loginUser.User!.Id!} logged in successfully with roles: {string.Join(", ", roles)}");

            var token = jwtService.GenerateToken(loginUser.User!.Id!, loginUser.User!.UserName!, roles.FirstOrDefault()!);
